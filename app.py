...
sheet = client.open_by_key("1hpwFpIyX8VCC25R3w6DWlTfThf-uSO7jvtd5jebIqxE").worksheet("Sheet1")
sheet1 = client.open_by_key("1hpwFpIyX8VCC25R3w6DWlTfThf-uSO7jvtd5jebIqxE").worksheet("leave")
sheet2 = client.open_by_key("1hpwFpIyX8VCC25R3w6DWlTfThf-uSO7jvtd5jebIqxE").worksheet("holiday")
sheet3 = client.open_by_key("1hpwFpIyX8VCC25R3w6DWlTfThf-uSO7jvtd5jebIqxE").worksheet("admindashboard")
sheet4 = client.open_by_key("1hpwFpIyX8VCC25R3w6DWlTfThf-uSO7jvtd5jebIqxE").worksheet("loginsheet")
sheet5 = client.open_by_key("1hpwFpIyX8VCC25R3w6DWlTfThf-uSO7jvtd5jebIqxE").worksheet("timesheet")
sheet6 = client.open_by_key("1hpwFpIyX8VCC25R3w6DWlTfThf-uSO7jvtd5jebIqxE").worksheet("holiday")
payslip_sheet = client.open_by_key("1hpwFpIyX8VCC25R3w6DWlTfThf-uSO7jvtd5jebIqxE").worksheet("payslip")
...
def employee_portal():
    try:
        data = get_sheet_data()
        # Show all payslips instead of filtering by status
        available_files = data  # Remove the status filter
...
@app.route('/request_payslip', methods=['POST'])
@login_required
def request_payslip():
    try:
        ...
        # Format the month-year to be more readable (e.g., "August 2025")
        try:
            month_year_formatted = datetime.strptime(month_year, "%Y-%m").strftime("%B %Y")
        except ValueError:
            month_year_formatted = month_year

        request_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        
        # Prepare the row data with explicit column mapping
        new_row = [
            employee_name,          # Column B (Employee Name)
            employee_id,            # Column C (Employee ID)
            month_year_formatted,   # Column D (Month-Year)
            additional_notes,       # Column E (Additional Notes)
            request_date,           # Column F (Request Date)
            'Pending',              # Column G (Status)
            '',                     # Column H (File Name)
            '',                     # Column I (Download Link)
            '',                     # Column J (Upload Date)
            ''                      # Column K (Archived Date)
        ]
...
def get_sheet_data():
    try:
        all_values = payslip_sheet.get_all_values()
        
        if len(all_values) < 1:
            return []
            
        # Define expected headers based on your actual spreadsheet columns
        headers = [
            'Employee Name',     # B
            'Employee ID',       # C
            'Month-Year',        # D
        ...
        for i, row in enumerate(all_values[1:], start=2):  # Skip header row
            ...
            record['row_id'] = i  # Add the row number for reference
            
            # Clean up status field
            if 'Status' in record:
                record['Status'] = record['Status'].strip().capitalize()
                if record['Status'] == 'Fulfilled':  # Fix common typo
